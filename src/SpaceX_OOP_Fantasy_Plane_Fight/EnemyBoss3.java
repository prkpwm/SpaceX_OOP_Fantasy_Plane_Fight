...
public class EnemyBoss3 extends Sprite {

    private List<MissileE> missilesE;

    public EnemyBoss3(int xE, int yE) {
    ...
    private void initSpaceShip() {
        ...
        loadImage("/image/Enemy/boss3_1.png");
        getImageDimensions();
        timeValue();
        a = time;
        TM = timeM;
    ...
    public void move() {
        if (Can_Move == true) {
            ...
            if (Math.abs(xPos - xE) <= 30 && Bullet == true && xE > 0) {
                //y = 800;        
                blood -= damage;
            } else if (Math.abs(timeM - TM) < 3 && Math.abs(timeM - TM) > 1) {
                System.out.println(Math.abs(xPos - xE));
                if (blood < 0) {
                    c++;
                    if (c <= 500) {
                        num += 1;
