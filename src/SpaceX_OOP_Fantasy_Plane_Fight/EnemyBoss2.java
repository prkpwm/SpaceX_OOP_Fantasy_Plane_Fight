...
public class EnemyBoss2 extends Sprite {

    private List<MissileE> missilesE;

    public EnemyBoss2(int xE, int yE) {
    ...
    private void initSpaceShip() {
        ...
        loadImage("/image/Enemy/boss2_1.png");
        getImageDimensions();
        timeValue();
        a = time;
        TM = timeM;
    ...
    public void move() {
        if (Can_Move == true) {
            ...
            if (Math.abs(xPos - xE) <= 30 && Bullet == true && xE > 0) {
                //y = 800;        
                blood -= damage;
            } else if (Math.abs(timeM - TM) < 2 && Math.abs(timeM - TM) > 0) {
